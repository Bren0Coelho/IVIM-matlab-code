function [reg_c,rho_c,eta_c] = l_corner(rho,eta,reg_param,U,s,Signal,method,M)
...
% Tikhonov=========================================================================
  if (strncmp(method,'Tikh',4) || strncmp(method,'tikh',4))
    ...
    % fminbnd(FUN, X1, X2) finds a local minimum X so that X1 < X < X2
    reg_c = fminbnd('lcfun', reg_param(min(gi+1,length(g))), reg_param(max(gi-1,1)), optimset('Display','off'),s,beta,xi); % Minimizer. Finds the parameter for the corner
      
    kappa_max = - lcfun(reg_c,s,beta,xi); % Maximum curvature.
    
    if (kappa_max < 0) %  If the curvature is negative everywhere, then define the leftmost point of the L-curve as the corner.
      ...
      if (m>n)
        ...
        rho_c = sqrt(rho_c^2 + norm(Signal0)^2); 
