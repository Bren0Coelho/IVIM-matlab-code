function [reg_corner,rho,eta,reg_param] = l_curve(U,sm,Signal,method,L,V)
  %L_CURVE Plot the L-curve and find its "corner".
  %
  % [reg_corner,rho,eta,reg_param] =
  %                  l_curve(U,s,M,method)
  %                  l_curve(U,sm,M,method)  ,  sm = [sigma,mu]
  %                  l_curve(U,s,M,method,L,V)
  %
  ...
  npt = 200;  % Number of points on the L-curve for Tikh and dsvd.
  smin_ratio = 16*eps;  % Smallest regularization parameter.
  
%% Inicialization==============================================================

  [m,n] = size(U);     % U is m x n
  [p,ps] = size(sm);
  ...
  beta = U'*Signal; 
  beta2 = norm(Signal)^2 - norm(beta)^2;
  ...
  xi = beta(1:p)./s;   %x_lsq = U'*Signal*V/s
  xi( isinf(xi) ) = 0;
...
% Métodos=======================================================================
  ...
  if (strncmp(method,'Tikh',4) || strncmp(method,'tikh',4))
    ...
    reg_param = eta; 
    s2 = s.^2;
    reg_param(npt) = max([s(p),s(1)*smin_ratio]);
    ratio = (s(1)/reg_param(npt))^(1/(npt-1));
    for i=npt-1:-1:1
      reg_param(i) = ratio*reg_param(i+1); 
    end
    for i=1:npt
      f = s2./(s2 + reg_param(i)^2);
      eta(i) = norm(f.*xi);                 %eta = ||x_lambda||2_^2 = (f*U'*Signal/s)^2
      rho(i) = norm((1-f).*beta(1:p));      %rho = ||A*x_lambda - Signal||2_^2 = ((1-f)*U'*Signal)^2
    end
...
%% Curvatura====================================================================
  ...
  if (locate)
    ...
    [reg_corner,rho_c,eta_c] = l_corner(rho,eta,reg_param,U,sm,Signal,method);
