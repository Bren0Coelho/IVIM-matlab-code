...
% [data, header] = readrec_V4_2(filename[, 'quiet'][, 'par'][, 'scale'])
%
...
%  returned.
% If 'scale' is provided, data is scaled using slope and intercept from
%  par bottom table.
%
...
% 20151007 JSG - add scale option, add code for multile b-values
%
...
%  3 = Phase, 16 = corbadump, 52 total types
%type1 =    unique(A(:,5));
% scanning sequence: 14 possible scan sequences
%type2 =    unique(A(:,6));
% combine image type and scanning sequence
typess = unique(A(:,6)*100+A(:,5));
dynamics = unique(A(:,3));
echoes =   unique(A(:,2));
phases =   unique(A(:,4));

if (header.par_version > 4)
    % Philips allows 512 directions and 32 bvalues max so dir and b-val
    % are combined using offest of 100
     bdir = unique(A(:,43)*100+A(:,42));
...
% progress bar
if ~quiet
    h = waitbar(0, sprintf('Reading in %s', filename)); 
end
...
for idx = 0:s-1
    ...
    ii = find(A(:,7)==idx);
    d = fread (fp, [header.nrows, header.ncols], 'int16');
    if scale
        %  Determine the rescale slope, intercept and other scaling factors.
        intercept = A(ii,iri) / A(ii,irs); % ri is usually 0 for magn data
        slope = 1.0 / A(ii,iss);
        dd = double(d) * slope + intercept;
    else
        dd = d;
    end
    if size(dd) == header.sizes(2:-1:1)
        if (header.par_version == 4.2)
            % v 4.2 - diffusion & ASL type dimension
            %   find(type1==A(ii,5)), find(type2==A(ii,6)),
            data(:,:, A(ii,1), find(echoes==A(ii,2)), find(dynamics==A(ii,3)),...
                find(typess==A(ii,6)*100+A(ii,5)), ...
                find(phases==A(ii,4)), find(bdir==A(ii,43)*100+A(ii,42)), ...
                find(ASL_type==A(ii,49))) = dd; %#ok<FNDSB>
        elseif (header.par_version >= 4)
            ...
            data(:,:, A(ii,1), find(echoes==A(ii,2)), find(dynamics==A(ii,3)),...
                find(typess==A(ii,6)*100+A(ii,5)), ...
                find(phases==A(ii,4))) = dd; %#ok<FNDSB>
        end
    else
        if ~quiet
...
while( 1 )
    ...
    line = strtrim(line);
    line = line(1:289); % drop contrast values if present
    line = str2num(line);%#ok<ST2NM>    
    if length(line) == 48
        line(end+1) = 1; %#ok<AGROW>
    end
    A(ii,:) = line; %#ok<AGROW> 
    ii = ii + 1;
end
...
header.tblcols = {'slice' 'echo' 'dynamic' 'card_phase' 'type', 'seq' ...
    'index'};
if (header.par_version > 4)
...
header.sizes = [header.nrows header.ncols header.nslices header.nechoes ...
    header.ndynamics length(unique(A(:,6)*100+A(:,5))) header.nphases];
header.dims = {'rows', 'cols', 'slices', 'echoes', ...
    'dynamics' 'type_seq' 'phases'};
if (header.par_version > 4)
    header.sizes = [header.sizes length(unique(A(:,43)*100+A(:,42)))]; %grad ori & val
...
if ~quiet
    fprintf(['Reading in %s:\n rows\t\t= %d\n columns\t= %d\n' ...
        ' slices\t\t= %d\n echoes\t\t= %d\n dynamics\t= %d\n' ...
        ' data types/scan seqs\t= %d\n card phases\t= %d\n'], ...
        filename_par, header.sizes(1:7));
    if( header.par_version >= 4)
        fprintf(' bdirs & vals\t= %d\n', header.sizes(8));
    end
    if( header.par_version == 4.2)
        fprintf(' ASL types\t= %d\n', header.sizes(9));
    end
